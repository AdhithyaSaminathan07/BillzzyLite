    ...
    <>
      ...
      <aside className={`fixed top-0 left-0 h-full w-64 flex flex-col bg-white z-40 lg:relative transform transition-transform duration-300 ease-in-out shadow-2xl lg:shadow-none lg:border-r ${isMobileOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
        ...
        <nav className="flex flex-1 flex-col space-y-2 p-4">
          ...
          <NavLink href="/billing" onClick={handleLinkClick}><CreditCard className="h-5 w-5" /><span>Billing</span></NavLink>
          <NavLink href="/billing-history" onClick={handleLinkClick}><Clock className="h-5 w-5" /><span>Billing History</span></NavLink>
          <NavLink href="/settings" onClick={handleLinkClick}><Settings className="h-5 w-5" /><span>Settings</span></NavLink>
        </nav>
