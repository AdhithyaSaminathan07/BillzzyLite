...
export function BottomNavBar() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-[#5a4fcf] z-40 h-20 flex items-center justify-between px-3 shadow-[0_-2px_8px_rgba(0,0,0,0.2)] rounded-t-2xl lg:hidden">
      <div className="flex flex-1 justify-evenly">{navItems.slice(0, 2).map((item) => (<NavLink key={item.href} {...item} />))}</div>
      <Link
        href="/billing"
        className={`relative bg-white text-[#5a4fcf] rounded-full p-4 shadow-xl transform transition-all duration-300 -translate-y-4 ${
          isBilling ? 'scale-110' : 'hover:scale-105'
        }`}
      >
        <ScanLine className="w-8 h-8" />
      </Link>
      <div className="flex flex-1 justify-evenly">{navItems.slice(2).map((item) => (<NavLink key={item.href} {...item} />))}</div>
    </nav>
  );
}