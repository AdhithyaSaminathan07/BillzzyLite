...
const API_ENDPOINTS = {
    SALES: (period: Period) => `/api/sales?period=${period.toLowerCase()}`,
...
export default function Dashboard() {
  ...
  const [salesData, setSalesData] = useState<SalesData>({ total: 0, cash: 0, qr: 0, bills: 0, lastUpdated: "" });
  const [activePeriod, setActivePeriod] = useState<Period>("Today");
  const [isSalesLoading, setIsSalesLoading] = useState(true);
  const [salesError, setSalesError] = useState<string | null>(null);

  // State for Inventory Summary
  const [inventorySummary, setInventorySummary] = useState<InventorySummary>({ inStock: 0, lowStock: 0, outOfStock: 0 });
  ...
  const fetchSales = useCallback(async () => {
    ...
    setSalesError(null);
    try {
      const res = await fetch(API_ENDPOINTS.SALES(activePeriod));
  ...
  const fetchInventorySummary = useCallback(async () => {
    ...
    try {
      ...
      const summary: InventorySummary = products.reduce((acc, product) => {
        ...
        else if (product.quantity <= threshold) acc.lowStock++;
        else acc.inStock++;
        return acc;
    ...
    return (
      <div className="h-full bg-gray-50 flex items-center justify-center p-4">
          <div className="flex flex-col items-center text-gray-400">
            <Loader2 className="w-6 h-6 animate-spin mb-2 text-[#5a4fcf]" /> 
    ...
    return (
      <div className="h-full bg-gray-50 flex items-center justify-center p-4">
          <div className="flex flex-col items-center text-red-500">
            ...
            <span className="text-sm">Please log in to view the dashboard.</span>
          </div>
      </div>
    );
  }
            ...
            {TABS.map((tab) => (
              <button
                ...
                className={`flex-1 py-1.5 text-xs font-semibold rounded-md transition-all ${
                  activePeriod === tab ? "bg-[#5a4fcf] text-white" : "text-gray-600"
                }`}
              >
                {tab}
              </button>
            ...
            <div className="space-y-2.5">
              ...
              <div className="bg-gradient-to-br from-orange-50 to-orange-100 p-2.5 rounded-lg border-2 border-orange-200">
                <div className="flex items-center justify-between">
                  ...
                  <svg className="w-6 h-6 text-orange-600" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" /></svg>
                </div>
              </div>
